...
# Cap simultaneous Gemini calls to stay inside the free-tier QPM limit.
# Raise to 6-8 if you are on a paid plan.
_GEMINI_SEM = threading.Semaphore(6)
...
6. GRAND TOTAL VERIFICATION — After extracting all line items, sum their totals and
   ...
   If there is a mismatch, flag it by adding a key "Total_Mismatch": true to the JSON."""
    ...
    prompt = (
        ...
        "NUMERIC AMOUNTS RULE (critical): All numeric/amount values must be plain numbers only. "
        "Handwritten invoices sometimes have a trailing dash after an amount (e.g. '750-') — "
        "this is a casual pen stroke, NOT a minus sign. Output only the number (e.g. 750, not 750-). "
        "Never output amounts with a trailing dash, trailing dot, or any non-numeric suffix.\n"
...
def process_invoice(image_data, selected_fields=None, custom_fields=None, source_text=None):
    ...
    # ── Level-3 parallelism: Gemini extraction and QR scan are independent ──
    # Neither needs the other's result. Fire both at the same time and join
    # once both finish.  _GEMINI_SEM caps the total number of in-flight Gemini
    # calls across all levels so we don't blow the QPM rate limit.
    def _call_gemini():
        with _GEMINI_SEM:
        ...
        b = re.search(r"(\{.*\})", resp_text, re.DOTALL)
        return json.loads(b.group(1).strip() if b else resp_text)

    with ThreadPoolExecutor(max_workers=2) as _exe:
        _g_fut = _exe.submit(_call_gemini)
        _q_fut = _exe.submit(extract_qr_code, image_data)
        invoice_data   = _g_fut.result()
        qr_code_string = _q_fut.result()
    ...
    def clean_numeric(value):
        """Strip trailing dash handwriting artefacts from numeric strings.
        e.g. '750-' → '750', '1,542.22-' → '1,542.22'
        A leading dash (genuine negative) is intentionally preserved."""
    ...
    for field in selected_fields:
        ...
        raw = find_value(invoice_data, field["aliases"], default)
        # Strip trailing-dash artefacts from amount fields
        new_invoice_data[field["key"]] = clean_numeric(raw) if field["type"] == "float" else raw
    ...
    # QR_Code_Present is derived from the concurrent qr_code_string result above.
    if qr_code_string:
...
def process_document(file_bytes, mime_type, filename, selected_fields, custom_fields, split_invoices):
    ...
    if not split_invoices:
        ...
        # ── Level-2 parallelism: each page is an independent Gemini call ──
        # Process all pages concurrently; _GEMINI_SEM inside process_invoice
        # ensures we never exceed 4 simultaneous API calls.
        def _proc_page(args):
            idx, img = args
            src = text_for(idx)
            rec = process_invoice(img, selected_fields, custom_fields, source_text=src)
            if rec:
                rec["_source_kind"] = detection["source_kind"]
                rec["_extraction_path"] = "digital_text+vision" if src else "vision"
            return idx, rec

        n = len(page_images)
        ordered = {}
        with ThreadPoolExecutor(max_workers=min(n, 5)) as exe:
            futs = {exe.submit(_proc_page, (i, img)): i for i, img in enumerate(page_images)}
            for fut in as_completed(futs):
                idx, rec = fut.result()
                if rec:
                    ordered[idx] = rec
        for idx in sorted(ordered):
            records.append(ordered[idx])
    ...
    if page_selection == "Project Overview":
        ...
        if st.button("Process Invoices"):
            if not selected_fields and not custom_fields:
                ...
                # ── Level-1 parallelism: each file is an independent pipeline call ──
                # Read all bytes in the main thread first — UploadedFile objects are
                # not safe to access from worker threads.
                file_payloads = [
                    {"name": f.name, "type": f.type, "data": f.read()}
                    for f in uploaded_files
                ]
                n_files = len(file_payloads)

                def _process_file(payload):
                    recs = process_document(
                    ...
                    return payload["name"], recs

                status.info(f"Processing {n_files} file(s) in parallel…")
                all_results = {}   # filename -> records list
                done_count = 0
                with ThreadPoolExecutor(max_workers=min(n_files, 4)) as exe:
                    futs = {exe.submit(_process_file, p): p["name"] for p in file_payloads}
                    for fut in as_completed(futs):
                        fname = futs[fut]
                        try:
                            _, recs = fut.result()
                            all_results[fname] = recs
                        except Exception as e:
                ...
                # Dedup and tag in original file order (main thread — no race condition)
                for payload in file_payloads:
                    fname = payload["name"]
                    for invoice_data in all_results.get(fname, []):
                        if not invoice_data:
                            continue
